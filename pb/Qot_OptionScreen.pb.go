// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        v7.35.0
// source: Qot_OptionScreen.proto

package pb

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// 期权市场品类 (枚举值与后台 FTCmdOptionMarket.MarketCategory 一致, 直接透传)
type MarketCategory int32

const (
	MarketCategory_MarketCategory_US_Stock  MarketCategory = 0 // 美股股票期权
	MarketCategory_MarketCategory_US_Index  MarketCategory = 1 // 美股指数期权
	MarketCategory_MarketCategory_US_Future MarketCategory = 2 // 美股期货期权
	MarketCategory_MarketCategory_HK_Stock  MarketCategory = 3 // 港股股票期权
	MarketCategory_MarketCategory_HK_Index  MarketCategory = 4 // 港股指数期权
	MarketCategory_MarketCategory_JP_Stock  MarketCategory = 5 // 日股股票期权
	MarketCategory_MarketCategory_JP_Index  MarketCategory = 6 // 日股指数期权
)

// Enum value maps for MarketCategory.
var (
	MarketCategory_name = map[int32]string{
		0: "MarketCategory_US_Stock",
		1: "MarketCategory_US_Index",
		2: "MarketCategory_US_Future",
		3: "MarketCategory_HK_Stock",
		4: "MarketCategory_HK_Index",
		5: "MarketCategory_JP_Stock",
		6: "MarketCategory_JP_Index",
	}
	MarketCategory_value = map[string]int32{
		"MarketCategory_US_Stock":  0,
		"MarketCategory_US_Index":  1,
		"MarketCategory_US_Future": 2,
		"MarketCategory_HK_Stock":  3,
		"MarketCategory_HK_Index":  4,
		"MarketCategory_JP_Stock":  5,
		"MarketCategory_JP_Index":  6,
	}
)

func (x MarketCategory) Enum() *MarketCategory {
	p := new(MarketCategory)
	*p = x
	return p
}

func (x MarketCategory) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (MarketCategory) Descriptor() protoreflect.EnumDescriptor {
	return file_Qot_OptionScreen_proto_enumTypes[0].Descriptor()
}

func (MarketCategory) Type() protoreflect.EnumType {
	return &file_Qot_OptionScreen_proto_enumTypes[0]
}

func (x MarketCategory) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Do not use.
func (x *MarketCategory) UnmarshalJSON(b []byte) error {
	num, err := protoimpl.X.UnmarshalJSONEnum(x.Descriptor(), b)
	if err != nil {
		return err
	}
	*x = MarketCategory(num)
	return nil
}

// Deprecated: Use MarketCategory.Descriptor instead.
func (MarketCategory) EnumDescriptor() ([]byte, []int) {
	return file_Qot_OptionScreen_proto_rawDescGZIP(), []int{0}
}

// 标的筛选因子类型
type UnderlyingIndicatorType int32

const (
	UnderlyingIndicatorType_UnderlyingIndicatorType_Unknown UnderlyingIndicatorType = 0 // 未知
	// 标的基本信息
	UnderlyingIndicatorType_UnderlyingIndicatorType_StockList UnderlyingIndicatorType = 101 // 【确切值】指定标的范围(股票ID列表)
	// ⚠️ 不支持: 后端 option_screener_svc 未实装, 传入会被 OpenD 入口拦截并返回错误
	//
	// Deprecated: Marked as deprecated in Qot_OptionScreen.proto.
	UnderlyingIndicatorType_UnderlyingIndicatorType_Plate     UnderlyingIndicatorType = 103 // 【已废弃】指定板块, 后端不支持
	UnderlyingIndicatorType_UnderlyingIndicatorType_IndexList UnderlyingIndicatorType = 106 // 【确切值】指定指数类型
	// 标的期权统计
	UnderlyingIndicatorType_UnderlyingIndicatorType_Volume        UnderlyingIndicatorType = 201 // 【范围】总成交量
	UnderlyingIndicatorType_UnderlyingIndicatorType_OpenInterest  UnderlyingIndicatorType = 202 // 【范围】总持仓量
	UnderlyingIndicatorType_UnderlyingIndicatorType_IV            UnderlyingIndicatorType = 203 // 【范围】标的IV
	UnderlyingIndicatorType_UnderlyingIndicatorType_HV            UnderlyingIndicatorType = 204 // 【范围】标的HV
	UnderlyingIndicatorType_UnderlyingIndicatorType_IVRank        UnderlyingIndicatorType = 205 // 【范围】标的IV Rank
	UnderlyingIndicatorType_UnderlyingIndicatorType_IVPercentile  UnderlyingIndicatorType = 206 // 【范围】标的IV Percentile
	UnderlyingIndicatorType_UnderlyingIndicatorType_IVChange      UnderlyingIndicatorType = 207 // 【范围】标的IV变化量
	UnderlyingIndicatorType_UnderlyingIndicatorType_IVChangeRatio UnderlyingIndicatorType = 208 // 【范围】标的IV变化率
	UnderlyingIndicatorType_UnderlyingIndicatorType_IVHVRatio     UnderlyingIndicatorType = 209 // 【范围】标的IV/HV
	UnderlyingIndicatorType_UnderlyingIndicatorType_IVHVSpread    UnderlyingIndicatorType = 210 // 【范围】标的IV-HV
	// 行情指标
	UnderlyingIndicatorType_UnderlyingIndicatorType_MarketCap   UnderlyingIndicatorType = 401 // 【范围】标的市值
	UnderlyingIndicatorType_UnderlyingIndicatorType_StockPrice  UnderlyingIndicatorType = 402 // 【范围】标的最新价
	UnderlyingIndicatorType_UnderlyingIndicatorType_ChangeRatio UnderlyingIndicatorType = 403 // 【范围】涨跌幅
)

// Enum value maps for UnderlyingIndicatorType.
var (
	UnderlyingIndicatorType_name = map[int32]string{
		0:   "UnderlyingIndicatorType_Unknown",
		101: "UnderlyingIndicatorType_StockList",
		103: "UnderlyingIndicatorType_Plate",
		106: "UnderlyingIndicatorType_IndexList",
		201: "UnderlyingIndicatorType_Volume",
		202: "UnderlyingIndicatorType_OpenInterest",
		203: "UnderlyingIndicatorType_IV",
		204: "UnderlyingIndicatorType_HV",
		205: "UnderlyingIndicatorType_IVRank",
		206: "UnderlyingIndicatorType_IVPercentile",
		207: "UnderlyingIndicatorType_IVChange",
		208: "UnderlyingIndicatorType_IVChangeRatio",
		209: "UnderlyingIndicatorType_IVHVRatio",
		210: "UnderlyingIndicatorType_IVHVSpread",
		401: "UnderlyingIndicatorType_MarketCap",
		402: "UnderlyingIndicatorType_StockPrice",
		403: "UnderlyingIndicatorType_ChangeRatio",
	}
	UnderlyingIndicatorType_value = map[string]int32{
		"UnderlyingIndicatorType_Unknown":       0,
		"UnderlyingIndicatorType_StockList":     101,
		"UnderlyingIndicatorType_Plate":         103,
		"UnderlyingIndicatorType_IndexList":     106,
		"UnderlyingIndicatorType_Volume":        201,
		"UnderlyingIndicatorType_OpenInterest":  202,
		"UnderlyingIndicatorType_IV":            203,
		"UnderlyingIndicatorType_HV":            204,
		"UnderlyingIndicatorType_IVRank":        205,
		"UnderlyingIndicatorType_IVPercentile":  206,
		"UnderlyingIndicatorType_IVChange":      207,
		"UnderlyingIndicatorType_IVChangeRatio": 208,
		"UnderlyingIndicatorType_IVHVRatio":     209,
		"UnderlyingIndicatorType_IVHVSpread":    210,
		"UnderlyingIndicatorType_MarketCap":     401,
		"UnderlyingIndicatorType_StockPrice":    402,
		"UnderlyingIndicatorType_ChangeRatio":   403,
	}
)

func (x UnderlyingIndicatorType) Enum() *UnderlyingIndicatorType {
	p := new(UnderlyingIndicatorType)
	*p = x
	return p
}

func (x UnderlyingIndicatorType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (UnderlyingIndicatorType) Descriptor() protoreflect.EnumDescriptor {
	return file_Qot_OptionScreen_proto_enumTypes[1].Descriptor()
}

func (UnderlyingIndicatorType) Type() protoreflect.EnumType {
	return &file_Qot_OptionScreen_proto_enumTypes[1]
}

func (x UnderlyingIndicatorType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Do not use.
func (x *UnderlyingIndicatorType) UnmarshalJSON(b []byte) error {
	num, err := protoimpl.X.UnmarshalJSONEnum(x.Descriptor(), b)
	if err != nil {
		return err
	}
	*x = UnderlyingIndicatorType(num)
	return nil
}

// Deprecated: Use UnderlyingIndicatorType.Descriptor instead.
func (UnderlyingIndicatorType) EnumDescriptor() ([]byte, []int) {
	return file_Qot_OptionScreen_proto_rawDescGZIP(), []int{1}
}

// 期权筛选因子类型
type OptionIndicatorType int32

const (
	OptionIndicatorType_OptionIndicatorType_Unknown OptionIndicatorType = 0 // 未知
	// 期权静态信息
	OptionIndicatorType_OptionIndicatorType_StrikePrice         OptionIndicatorType = 1001 // 【范围】行权价
	OptionIndicatorType_OptionIndicatorType_LeftDay             OptionIndicatorType = 1002 // 【范围】距离到期日天数
	OptionIndicatorType_OptionIndicatorType_OptionType          OptionIndicatorType = 1003 // 【确切值】期权类型(CALL/PUT)
	OptionIndicatorType_OptionIndicatorType_ExerciseType        OptionIndicatorType = 1004 // 【确切值】行权方式(美式/欧式)
	OptionIndicatorType_OptionIndicatorType_ExpirationType      OptionIndicatorType = 1005 // 【确切值】到期类型(周/月/季)
	OptionIndicatorType_OptionIndicatorType_StrikeDateTimestamp OptionIndicatorType = 1007 // 【确切值】到期日时间戳(秒)
	// 期权实时行情
	OptionIndicatorType_OptionIndicatorType_InTheMoney            OptionIndicatorType = 2001 // 【确切值】价内/价外(0:价外,1:价内)
	OptionIndicatorType_OptionIndicatorType_Price                 OptionIndicatorType = 2002 // 【范围】期权价格
	OptionIndicatorType_OptionIndicatorType_MidPrice              OptionIndicatorType = 2003 // 【范围】期权中间价
	OptionIndicatorType_OptionIndicatorType_BidPrice              OptionIndicatorType = 2004 // 【范围】买价
	OptionIndicatorType_OptionIndicatorType_AskPrice              OptionIndicatorType = 2005 // 【范围】卖价
	OptionIndicatorType_OptionIndicatorType_BidAskSpread          OptionIndicatorType = 2006 // 【范围】买卖价差
	OptionIndicatorType_OptionIndicatorType_BidVolume             OptionIndicatorType = 2007 // 【范围】买量
	OptionIndicatorType_OptionIndicatorType_AskVolume             OptionIndicatorType = 2008 // 【范围】卖量
	OptionIndicatorType_OptionIndicatorType_BidAskVolumeRatio     OptionIndicatorType = 2009 // 【范围】买卖量比
	OptionIndicatorType_OptionIndicatorType_ChangeRatio           OptionIndicatorType = 2010 // 【范围】涨跌幅
	OptionIndicatorType_OptionIndicatorType_Volume                OptionIndicatorType = 2011 // 【范围】成交量
	OptionIndicatorType_OptionIndicatorType_Turnover              OptionIndicatorType = 2012 // 【范围】成交额
	OptionIndicatorType_OptionIndicatorType_OpenInterest          OptionIndicatorType = 2013 // 【范围】持仓量
	OptionIndicatorType_OptionIndicatorType_OpenInterestMarketCap OptionIndicatorType = 2014 // 【范围】持仓市值
	OptionIndicatorType_OptionIndicatorType_VolOIRatio            OptionIndicatorType = 2018 // 【范围】成交量/持仓量
	// ⚠️ filter 不支持(后端 option_screener_svc 未实装, 入口拦截报错); sort/retrieve 可用
	OptionIndicatorType_OptionIndicatorType_Premium OptionIndicatorType = 2021 // 【范围】权利金, 仅 sort/retrieve
	// 期权分析指标
	OptionIndicatorType_OptionIndicatorType_ImpliedVolatility     OptionIndicatorType = 3001 // 【范围】隐含波动率
	OptionIndicatorType_OptionIndicatorType_HistoryVolatility     OptionIndicatorType = 3002 // 【范围】历史波动率
	OptionIndicatorType_OptionIndicatorType_IVHVRatio             OptionIndicatorType = 3003 // 【范围】IV/HV
	OptionIndicatorType_OptionIndicatorType_Delta                 OptionIndicatorType = 3004 // 【范围】Delta
	OptionIndicatorType_OptionIndicatorType_Gamma                 OptionIndicatorType = 3005 // 【范围】Gamma
	OptionIndicatorType_OptionIndicatorType_Vega                  OptionIndicatorType = 3006 // 【范围】Vega
	OptionIndicatorType_OptionIndicatorType_Theta                 OptionIndicatorType = 3007 // 【范围】Theta
	OptionIndicatorType_OptionIndicatorType_Rho                   OptionIndicatorType = 3008 // 【范围】Rho
	OptionIndicatorType_OptionIndicatorType_LeverageRatio         OptionIndicatorType = 3009 // 【范围】杠杆比率
	OptionIndicatorType_OptionIndicatorType_EffectiveGearing      OptionIndicatorType = 3010 // 【范围】有效杠杆
	OptionIndicatorType_OptionIndicatorType_BuyToBep              OptionIndicatorType = 3011 // 【范围】买入到盈亏平衡点比率
	OptionIndicatorType_OptionIndicatorType_SellToBep             OptionIndicatorType = 3012 // 【范围】卖出到盈亏平衡点比率
	OptionIndicatorType_OptionIndicatorType_BuyProfitProbability  OptionIndicatorType = 3013 // 【范围】买入盈利概率
	OptionIndicatorType_OptionIndicatorType_SellProfitProbability OptionIndicatorType = 3014 // 【范围】卖出盈利概率
	OptionIndicatorType_OptionIndicatorType_IntrinsicValuePer     OptionIndicatorType = 3015 // 【范围】内在价值百分比
	OptionIndicatorType_OptionIndicatorType_TimeValuePer          OptionIndicatorType = 3016 // 【范围】时间价值百分比
	OptionIndicatorType_OptionIndicatorType_ITMDegree             OptionIndicatorType = 3017 // 【范围】价内程度
	OptionIndicatorType_OptionIndicatorType_OTMDegree             OptionIndicatorType = 3018 // 【范围】价外程度
	OptionIndicatorType_OptionIndicatorType_ITMProbability        OptionIndicatorType = 3019 // 【范围】价内概率
	OptionIndicatorType_OptionIndicatorType_OTMProbability        OptionIndicatorType = 3020 // 【范围】价外概率
	OptionIndicatorType_OptionIndicatorType_SellAnnualizedReturn  OptionIndicatorType = 3021 // 【范围】卖出年化收益率
	OptionIndicatorType_OptionIndicatorType_IntervalReturn        OptionIndicatorType = 3022 // 【范围】卖出区间收益率
	// ⚠️ 已废弃: 不支持 filter/sort/retrieve, 传入会报错。新代码请使用 BuyToBep(3011)。
	//
	// Deprecated: Marked as deprecated in Qot_OptionScreen.proto.
	OptionIndicatorType_OptionIndicatorType_BuyBreakEvenPoint OptionIndicatorType = 3023 // 已废弃, 传入会报错
)

// Enum value maps for OptionIndicatorType.
var (
	OptionIndicatorType_name = map[int32]string{
		0:    "OptionIndicatorType_Unknown",
		1001: "OptionIndicatorType_StrikePrice",
		1002: "OptionIndicatorType_LeftDay",
		1003: "OptionIndicatorType_OptionType",
		1004: "OptionIndicatorType_ExerciseType",
		1005: "OptionIndicatorType_ExpirationType",
		1007: "OptionIndicatorType_StrikeDateTimestamp",
		2001: "OptionIndicatorType_InTheMoney",
		2002: "OptionIndicatorType_Price",
		2003: "OptionIndicatorType_MidPrice",
		2004: "OptionIndicatorType_BidPrice",
		2005: "OptionIndicatorType_AskPrice",
		2006: "OptionIndicatorType_BidAskSpread",
		2007: "OptionIndicatorType_BidVolume",
		2008: "OptionIndicatorType_AskVolume",
		2009: "OptionIndicatorType_BidAskVolumeRatio",
		2010: "OptionIndicatorType_ChangeRatio",
		2011: "OptionIndicatorType_Volume",
		2012: "OptionIndicatorType_Turnover",
		2013: "OptionIndicatorType_OpenInterest",
		2014: "OptionIndicatorType_OpenInterestMarketCap",
		2018: "OptionIndicatorType_VolOIRatio",
		2021: "OptionIndicatorType_Premium",
		3001: "OptionIndicatorType_ImpliedVolatility",
		3002: "OptionIndicatorType_HistoryVolatility",
		3003: "OptionIndicatorType_IVHVRatio",
		3004: "OptionIndicatorType_Delta",
		3005: "OptionIndicatorType_Gamma",
		3006: "OptionIndicatorType_Vega",
		3007: "OptionIndicatorType_Theta",
		3008: "OptionIndicatorType_Rho",
		3009: "OptionIndicatorType_LeverageRatio",
		3010: "OptionIndicatorType_EffectiveGearing",
		3011: "OptionIndicatorType_BuyToBep",
		3012: "OptionIndicatorType_SellToBep",
		3013: "OptionIndicatorType_BuyProfitProbability",
		3014: "OptionIndicatorType_SellProfitProbability",
		3015: "OptionIndicatorType_IntrinsicValuePer",
		3016: "OptionIndicatorType_TimeValuePer",
		3017: "OptionIndicatorType_ITMDegree",
		3018: "OptionIndicatorType_OTMDegree",
		3019: "OptionIndicatorType_ITMProbability",
		3020: "OptionIndicatorType_OTMProbability",
		3021: "OptionIndicatorType_SellAnnualizedReturn",
		3022: "OptionIndicatorType_IntervalReturn",
		3023: "OptionIndicatorType_BuyBreakEvenPoint",
	}
	OptionIndicatorType_value = map[string]int32{
		"OptionIndicatorType_Unknown":               0,
		"OptionIndicatorType_StrikePrice":           1001,
		"OptionIndicatorType_LeftDay":               1002,
		"OptionIndicatorType_OptionType":            1003,
		"OptionIndicatorType_ExerciseType":          1004,
		"OptionIndicatorType_ExpirationType":        1005,
		"OptionIndicatorType_StrikeDateTimestamp":   1007,
		"OptionIndicatorType_InTheMoney":            2001,
		"OptionIndicatorType_Price":                 2002,
		"OptionIndicatorType_MidPrice":              2003,
		"OptionIndicatorType_BidPrice":              2004,
		"OptionIndicatorType_AskPrice":              2005,
		"OptionIndicatorType_BidAskSpread":          2006,
		"OptionIndicatorType_BidVolume":             2007,
		"OptionIndicatorType_AskVolume":             2008,
		"OptionIndicatorType_BidAskVolumeRatio":     2009,
		"OptionIndicatorType_ChangeRatio":           2010,
		"OptionIndicatorType_Volume":                2011,
		"OptionIndicatorType_Turnover":              2012,
		"OptionIndicatorType_OpenInterest":          2013,
		"OptionIndicatorType_OpenInterestMarketCap": 2014,
		"OptionIndicatorType_VolOIRatio":            2018,
		"OptionIndicatorType_Premium":               2021,
		"OptionIndicatorType_ImpliedVolatility":     3001,
		"OptionIndicatorType_HistoryVolatility":     3002,
		"OptionIndicatorType_IVHVRatio":             3003,
		"OptionIndicatorType_Delta":                 3004,
		"OptionIndicatorType_Gamma":                 3005,
		"OptionIndicatorType_Vega":                  3006,
		"OptionIndicatorType_Theta":                 3007,
		"OptionIndicatorType_Rho":                   3008,
		"OptionIndicatorType_LeverageRatio":         3009,
		"OptionIndicatorType_EffectiveGearing":      3010,
		"OptionIndicatorType_BuyToBep":              3011,
		"OptionIndicatorType_SellToBep":             3012,
		"OptionIndicatorType_BuyProfitProbability":  3013,
		"OptionIndicatorType_SellProfitProbability": 3014,
		"OptionIndicatorType_IntrinsicValuePer":     3015,
		"OptionIndicatorType_TimeValuePer":          3016,
		"OptionIndicatorType_ITMDegree":             3017,
		"OptionIndicatorType_OTMDegree":             3018,
		"OptionIndicatorType_ITMProbability":        3019,
		"OptionIndicatorType_OTMProbability":        3020,
		"OptionIndicatorType_SellAnnualizedReturn":  3021,
		"OptionIndicatorType_IntervalReturn":        3022,
		"OptionIndicatorType_BuyBreakEvenPoint":     3023,
	}
)

func (x OptionIndicatorType) Enum() *OptionIndicatorType {
	p := new(OptionIndicatorType)
	*p = x
	return p
}

func (x OptionIndicatorType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (OptionIndicatorType) Descriptor() protoreflect.EnumDescriptor {
	return file_Qot_OptionScreen_proto_enumTypes[2].Descriptor()
}

func (OptionIndicatorType) Type() protoreflect.EnumType {
	return &file_Qot_OptionScreen_proto_enumTypes[2]
}

func (x OptionIndicatorType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Do not use.
func (x *OptionIndicatorType) UnmarshalJSON(b []byte) error {
	num, err := protoimpl.X.UnmarshalJSONEnum(x.Descriptor(), b)
	if err != nil {
		return err
	}
	*x = OptionIndicatorType(num)
	return nil
}

// Deprecated: Use OptionIndicatorType.Descriptor instead.
func (OptionIndicatorType) EnumDescriptor() ([]byte, []int) {
	return file_Qot_OptionScreen_proto_rawDescGZIP(), []int{2}
}

type OptionScreenBoundary struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Value         *float64               `protobuf:"fixed64,1,req,name=value" json:"value,omitempty"`      // 边界值 (double, 用户直接传原始值)
	Includes      *bool                  `protobuf:"varint,2,req,name=includes" json:"includes,omitempty"` // 是否包含此值
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *OptionScreenBoundary) Reset() {
	*x = OptionScreenBoundary{}
	mi := &file_Qot_OptionScreen_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *OptionScreenBoundary) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OptionScreenBoundary) ProtoMessage() {}

func (x *OptionScreenBoundary) ProtoReflect() protoreflect.Message {
	mi := &file_Qot_OptionScreen_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OptionScreenBoundary.ProtoReflect.Descriptor instead.
func (*OptionScreenBoundary) Descriptor() ([]byte, []int) {
	return file_Qot_OptionScreen_proto_rawDescGZIP(), []int{0}
}

func (x *OptionScreenBoundary) GetValue() float64 {
	if x != nil && x.Value != nil {
		return *x.Value
	}
	return 0
}

func (x *OptionScreenBoundary) GetIncludes() bool {
	if x != nil && x.Includes != nil {
		return *x.Includes
	}
	return false
}

type OptionScreenInterval struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	FilterMin     *OptionScreenBoundary  `protobuf:"bytes,1,opt,name=filterMin" json:"filterMin,omitempty"` // 区间下限
	FilterMax     *OptionScreenBoundary  `protobuf:"bytes,2,opt,name=filterMax" json:"filterMax,omitempty"` // 区间上限
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *OptionScreenInterval) Reset() {
	*x = OptionScreenInterval{}
	mi := &file_Qot_OptionScreen_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *OptionScreenInterval) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OptionScreenInterval) ProtoMessage() {}

func (x *OptionScreenInterval) ProtoReflect() protoreflect.Message {
	mi := &file_Qot_OptionScreen_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OptionScreenInterval.ProtoReflect.Descriptor instead.
func (*OptionScreenInterval) Descriptor() ([]byte, []int) {
	return file_Qot_OptionScreen_proto_rawDescGZIP(), []int{1}
}

func (x *OptionScreenInterval) GetFilterMin() *OptionScreenBoundary {
	if x != nil {
		return x.FilterMin
	}
	return nil
}

func (x *OptionScreenInterval) GetFilterMax() *OptionScreenBoundary {
	if x != nil {
		return x.FilterMax
	}
	return nil
}

type IndicatorValue struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ValueList     []int64                `protobuf:"varint,1,rep,name=valueList" json:"valueList,omitempty"`        // 确切值列表(用于枚举型筛选, 无倍率)
	ValueInterval *OptionScreenInterval  `protobuf:"bytes,2,opt,name=valueInterval" json:"valueInterval,omitempty"` // 值区间(用于范围型筛选, double 值)
	StrValueList  []string               `protobuf:"bytes,3,rep,name=strValueList" json:"strValueList,omitempty"`   // 字符串确切值列表(用于PLATE等, 如 ["BK1001"])
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *IndicatorValue) Reset() {
	*x = IndicatorValue{}
	mi := &file_Qot_OptionScreen_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *IndicatorValue) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*IndicatorValue) ProtoMessage() {}

func (x *IndicatorValue) ProtoReflect() protoreflect.Message {
	mi := &file_Qot_OptionScreen_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use IndicatorValue.ProtoReflect.Descriptor instead.
func (*IndicatorValue) Descriptor() ([]byte, []int) {
	return file_Qot_OptionScreen_proto_rawDescGZIP(), []int{2}
}

func (x *IndicatorValue) GetValueList() []int64 {
	if x != nil {
		return x.ValueList
	}
	return nil
}

func (x *IndicatorValue) GetValueInterval() *OptionScreenInterval {
	if x != nil {
		return x.ValueInterval
	}
	return nil
}

func (x *IndicatorValue) GetStrValueList() []string {
	if x != nil {
		return x.StrValueList
	}
	return nil
}

// 标的筛选条件
type UnderlyingIndicator struct {
	state          protoimpl.MessageState   `protogen:"open.v1"`
	IndicatorType  *UnderlyingIndicatorType `protobuf:"varint,1,req,name=indicatorType,enum=futupb.UnderlyingIndicatorType" json:"indicatorType,omitempty"` // UnderlyingIndicatorType
	IndicatorValue *IndicatorValue          `protobuf:"bytes,2,opt,name=indicatorValue" json:"indicatorValue,omitempty"`                                    // 筛选条件
	PlateList      []*OptionPlate           `protobuf:"bytes,4,rep,name=plateList" json:"plateList,omitempty"`                                              // 板块列表, 仅PLATE(103)类型使用
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *UnderlyingIndicator) Reset() {
	*x = UnderlyingIndicator{}
	mi := &file_Qot_OptionScreen_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UnderlyingIndicator) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UnderlyingIndicator) ProtoMessage() {}

func (x *UnderlyingIndicator) ProtoReflect() protoreflect.Message {
	mi := &file_Qot_OptionScreen_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UnderlyingIndicator.ProtoReflect.Descriptor instead.
func (*UnderlyingIndicator) Descriptor() ([]byte, []int) {
	return file_Qot_OptionScreen_proto_rawDescGZIP(), []int{3}
}

func (x *UnderlyingIndicator) GetIndicatorType() UnderlyingIndicatorType {
	if x != nil && x.IndicatorType != nil {
		return *x.IndicatorType
	}
	return UnderlyingIndicatorType_UnderlyingIndicatorType_Unknown
}

func (x *UnderlyingIndicator) GetIndicatorValue() *IndicatorValue {
	if x != nil {
		return x.IndicatorValue
	}
	return nil
}

func (x *UnderlyingIndicator) GetPlateList() []*OptionPlate {
	if x != nil {
		return x.PlateList
	}
	return nil
}

// 期权筛选条件
type OptionIndicator struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	IndicatorType  *OptionIndicatorType   `protobuf:"varint,1,req,name=indicatorType,enum=futupb.OptionIndicatorType" json:"indicatorType,omitempty"` // OptionIndicatorType
	IndicatorValue *IndicatorValue        `protobuf:"bytes,2,opt,name=indicatorValue" json:"indicatorValue,omitempty"`                                // 筛选条件
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *OptionIndicator) Reset() {
	*x = OptionIndicator{}
	mi := &file_Qot_OptionScreen_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *OptionIndicator) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OptionIndicator) ProtoMessage() {}

func (x *OptionIndicator) ProtoReflect() protoreflect.Message {
	mi := &file_Qot_OptionScreen_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OptionIndicator.ProtoReflect.Descriptor instead.
func (*OptionIndicator) Descriptor() ([]byte, []int) {
	return file_Qot_OptionScreen_proto_rawDescGZIP(), []int{4}
}

func (x *OptionIndicator) GetIndicatorType() OptionIndicatorType {
	if x != nil && x.IndicatorType != nil {
		return *x.IndicatorType
	}
	return OptionIndicatorType_OptionIndicatorType_Unknown
}

func (x *OptionIndicator) GetIndicatorValue() *IndicatorValue {
	if x != nil {
		return x.IndicatorValue
	}
	return nil
}

// 筛选条件组(组内OR, 组间AND)
type FilterGroup struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	UnderlyingList []*UnderlyingIndicator `protobuf:"bytes,1,rep,name=underlyingList" json:"underlyingList,omitempty"` // 标的筛选条件
	OptionList     []*OptionIndicator     `protobuf:"bytes,2,rep,name=optionList" json:"optionList,omitempty"`         // 期权筛选条件
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *FilterGroup) Reset() {
	*x = FilterGroup{}
	mi := &file_Qot_OptionScreen_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *FilterGroup) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*FilterGroup) ProtoMessage() {}

func (x *FilterGroup) ProtoReflect() protoreflect.Message {
	mi := &file_Qot_OptionScreen_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use FilterGroup.ProtoReflect.Descriptor instead.
func (*FilterGroup) Descriptor() ([]byte, []int) {
	return file_Qot_OptionScreen_proto_rawDescGZIP(), []int{5}
}

func (x *FilterGroup) GetUnderlyingList() []*UnderlyingIndicator {
	if x != nil {
		return x.UnderlyingList
	}
	return nil
}

func (x *FilterGroup) GetOptionList() []*OptionIndicator {
	if x != nil {
		return x.OptionList
	}
	return nil
}

// 板块
type OptionPlate struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ParentPlateId *string                `protobuf:"bytes,1,opt,name=parentPlateId" json:"parentPlateId,omitempty"` // 父板块ID (如 "BK1000")
	PlateIdList   []string               `protobuf:"bytes,2,rep,name=plateIdList" json:"plateIdList,omitempty"`     // 板块ID列表 (并集关系, 如 ["BK1001","BK1002"])
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *OptionPlate) Reset() {
	*x = OptionPlate{}
	mi := &file_Qot_OptionScreen_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *OptionPlate) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OptionPlate) ProtoMessage() {}

func (x *OptionPlate) ProtoReflect() protoreflect.Message {
	mi := &file_Qot_OptionScreen_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OptionPlate.ProtoReflect.Descriptor instead.
func (*OptionPlate) Descriptor() ([]byte, []int) {
	return file_Qot_OptionScreen_proto_rawDescGZIP(), []int{6}
}

func (x *OptionPlate) GetParentPlateId() string {
	if x != nil && x.ParentPlateId != nil {
		return *x.ParentPlateId
	}
	return ""
}

func (x *OptionPlate) GetPlateIdList() []string {
	if x != nil {
		return x.PlateIdList
	}
	return nil
}

// 排序
type OptionScreenSort struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	IndicatorType *OptionIndicatorType   `protobuf:"varint,1,req,name=indicatorType,enum=futupb.OptionIndicatorType" json:"indicatorType,omitempty"` // OptionIndicatorType(排序字段)
	Direction     *int32                 `protobuf:"varint,2,req,name=direction" json:"direction,omitempty"`                                         // 0=升序, 1=降序
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *OptionScreenSort) Reset() {
	*x = OptionScreenSort{}
	mi := &file_Qot_OptionScreen_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *OptionScreenSort) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OptionScreenSort) ProtoMessage() {}

func (x *OptionScreenSort) ProtoReflect() protoreflect.Message {
	mi := &file_Qot_OptionScreen_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OptionScreenSort.ProtoReflect.Descriptor instead.
func (*OptionScreenSort) Descriptor() ([]byte, []int) {
	return file_Qot_OptionScreen_proto_rawDescGZIP(), []int{7}
}

func (x *OptionScreenSort) GetIndicatorType() OptionIndicatorType {
	if x != nil && x.IndicatorType != nil {
		return *x.IndicatorType
	}
	return OptionIndicatorType_OptionIndicatorType_Unknown
}

func (x *OptionScreenSort) GetDirection() int32 {
	if x != nil && x.Direction != nil {
		return *x.Direction
	}
	return 0
}

// 标的统计信息
type UnderlyingInfo struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	StockID       *uint64                `protobuf:"varint,1,opt,name=stockID" json:"stockID,omitempty"`            // 标的股票ID
	Iv            *float64               `protobuf:"fixed64,2,opt,name=iv" json:"iv,omitempty"`                     // 标的IV
	Hv            *float64               `protobuf:"fixed64,3,opt,name=hv" json:"hv,omitempty"`                     // 标的HV
	IvRank        *float64               `protobuf:"fixed64,4,opt,name=ivRank" json:"ivRank,omitempty"`             // IV Rank
	IvPercentile  *float64               `protobuf:"fixed64,5,opt,name=ivPercentile" json:"ivPercentile,omitempty"` // IV Percentile
	MarketCap     *float64               `protobuf:"fixed64,6,opt,name=marketCap" json:"marketCap,omitempty"`       // 市值
	Price         *float64               `protobuf:"fixed64,7,opt,name=price" json:"price,omitempty"`               // 标的价格
	ChangeRatio   *float64               `protobuf:"fixed64,8,opt,name=changeRatio" json:"changeRatio,omitempty"`   // 涨跌幅
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UnderlyingInfo) Reset() {
	*x = UnderlyingInfo{}
	mi := &file_Qot_OptionScreen_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UnderlyingInfo) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UnderlyingInfo) ProtoMessage() {}

func (x *UnderlyingInfo) ProtoReflect() protoreflect.Message {
	mi := &file_Qot_OptionScreen_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UnderlyingInfo.ProtoReflect.Descriptor instead.
func (*UnderlyingInfo) Descriptor() ([]byte, []int) {
	return file_Qot_OptionScreen_proto_rawDescGZIP(), []int{8}
}

func (x *UnderlyingInfo) GetStockID() uint64 {
	if x != nil && x.StockID != nil {
		return *x.StockID
	}
	return 0
}

func (x *UnderlyingInfo) GetIv() float64 {
	if x != nil && x.Iv != nil {
		return *x.Iv
	}
	return 0
}

func (x *UnderlyingInfo) GetHv() float64 {
	if x != nil && x.Hv != nil {
		return *x.Hv
	}
	return 0
}

func (x *UnderlyingInfo) GetIvRank() float64 {
	if x != nil && x.IvRank != nil {
		return *x.IvRank
	}
	return 0
}

func (x *UnderlyingInfo) GetIvPercentile() float64 {
	if x != nil && x.IvPercentile != nil {
		return *x.IvPercentile
	}
	return 0
}

func (x *UnderlyingInfo) GetMarketCap() float64 {
	if x != nil && x.MarketCap != nil {
		return *x.MarketCap
	}
	return 0
}

func (x *UnderlyingInfo) GetPrice() float64 {
	if x != nil && x.Price != nil {
		return *x.Price
	}
	return 0
}

func (x *UnderlyingInfo) GetChangeRatio() float64 {
	if x != nil && x.ChangeRatio != nil {
		return *x.ChangeRatio
	}
	return 0
}

// 期权数据项
type OptionScreenItem struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	Security       *Security              `protobuf:"bytes,1,opt,name=security" json:"security,omitempty"`              // 期权证券信息
	OptionName     *string                `protobuf:"bytes,2,opt,name=optionName" json:"optionName,omitempty"`          // 期权名称
	StrikePrice    *float64               `protobuf:"fixed64,3,opt,name=strikePrice" json:"strikePrice,omitempty"`      // 行权价
	StrikeDate     *int64                 `protobuf:"varint,4,opt,name=strikeDate" json:"strikeDate,omitempty"`         // 行权日(格式YYYYMMDD)
	OptionType     *int32                 `protobuf:"varint,5,opt,name=optionType" json:"optionType,omitempty"`         // 期权类型(CALL/PUT)
	ExerciseType   *int32                 `protobuf:"varint,6,opt,name=exerciseType" json:"exerciseType,omitempty"`     // 行权方式
	ExpirationType *int32                 `protobuf:"varint,7,opt,name=expirationType" json:"expirationType,omitempty"` // 到期类型
	InTheMoney     *int32                 `protobuf:"varint,8,opt,name=inTheMoney" json:"inTheMoney,omitempty"`         // 价内/价外(0:价外,1:价内)
	LeftDay        *int32                 `protobuf:"varint,9,opt,name=leftDay" json:"leftDay,omitempty"`               // 距到期天数
	// 行情数据
	Price                 *float64 `protobuf:"fixed64,20,opt,name=price" json:"price,omitempty"`                                 // 期权价格
	MidPrice              *float64 `protobuf:"fixed64,21,opt,name=midPrice" json:"midPrice,omitempty"`                           // 中间价
	BidPrice              *float64 `protobuf:"fixed64,22,opt,name=bidPrice" json:"bidPrice,omitempty"`                           // 买价
	AskPrice              *float64 `protobuf:"fixed64,23,opt,name=askPrice" json:"askPrice,omitempty"`                           // 卖价
	BidAskSpread          *float64 `protobuf:"fixed64,24,opt,name=bidAskSpread" json:"bidAskSpread,omitempty"`                   // 买卖价差
	BidVolume             *int64   `protobuf:"varint,25,opt,name=bidVolume" json:"bidVolume,omitempty"`                          // 买量
	AskVolume             *int64   `protobuf:"varint,26,opt,name=askVolume" json:"askVolume,omitempty"`                          // 卖量
	ChangeRatio           *float64 `protobuf:"fixed64,27,opt,name=changeRatio" json:"changeRatio,omitempty"`                     // 涨跌幅
	Volume                *int64   `protobuf:"varint,28,opt,name=volume" json:"volume,omitempty"`                                // 成交量
	Turnover              *float64 `protobuf:"fixed64,29,opt,name=turnover" json:"turnover,omitempty"`                           // 成交额
	OpenInterest          *int64   `protobuf:"varint,30,opt,name=openInterest" json:"openInterest,omitempty"`                    // 持仓量
	BidAskVolumeRatio     *float64 `protobuf:"fixed64,31,opt,name=bidAskVolumeRatio" json:"bidAskVolumeRatio,omitempty"`         // 买卖量比
	OpenInterestMarketCap *float64 `protobuf:"fixed64,32,opt,name=openInterestMarketCap" json:"openInterestMarketCap,omitempty"` // 持仓市值
	VolOIRatio            *float64 `protobuf:"fixed64,33,opt,name=volOIRatio" json:"volOIRatio,omitempty"`                       // 成交量/持仓量
	Premium               *float64 `protobuf:"fixed64,34,opt,name=premium" json:"premium,omitempty"`                             // 权利金
	// Greeks
	ImpliedVolatility *float64 `protobuf:"fixed64,40,opt,name=impliedVolatility" json:"impliedVolatility,omitempty"` // 隐含波动率
	Delta             *float64 `protobuf:"fixed64,41,opt,name=delta" json:"delta,omitempty"`                         // Delta
	Gamma             *float64 `protobuf:"fixed64,42,opt,name=gamma" json:"gamma,omitempty"`                         // Gamma
	Vega              *float64 `protobuf:"fixed64,43,opt,name=vega" json:"vega,omitempty"`                           // Vega
	Theta             *float64 `protobuf:"fixed64,44,opt,name=theta" json:"theta,omitempty"`                         // Theta
	Rho               *float64 `protobuf:"fixed64,45,opt,name=rho" json:"rho,omitempty"`                             // Rho
	LeverageRatio     *float64 `protobuf:"fixed64,46,opt,name=leverageRatio" json:"leverageRatio,omitempty"`         // 杠杆比率
	EffectiveGearing  *float64 `protobuf:"fixed64,47,opt,name=effectiveGearing" json:"effectiveGearing,omitempty"`   // 有效杠杆
	ItmProbability    *float64 `protobuf:"fixed64,48,opt,name=itmProbability" json:"itmProbability,omitempty"`       // 价内概率
	// ⚠️ 历史字段: 后端未返回对应数据, 已弃用, 始终不填充
	// 新代码请用 buyToBep (字段 53)
	//
	// Deprecated: Marked as deprecated in Qot_OptionScreen.proto.
	BuyBreakEvenPoint *float64 `protobuf:"fixed64,49,opt,name=buyBreakEvenPoint" json:"buyBreakEvenPoint,omitempty"` // 已弃用, 始终为空
	// 标的信息
	UnderlyingInfo *UnderlyingInfo `protobuf:"bytes,50,opt,name=underlyingInfo" json:"underlyingInfo,omitempty"` // 标的信息
	// 期权分析指标 (续)
	HistoryVolatility     *float64 `protobuf:"fixed64,51,opt,name=historyVolatility" json:"historyVolatility,omitempty"`         // 历史波动率
	IvHvRatio             *float64 `protobuf:"fixed64,52,opt,name=ivHvRatio" json:"ivHvRatio,omitempty"`                         // IV/HV
	BuyToBep              *float64 `protobuf:"fixed64,53,opt,name=buyToBep" json:"buyToBep,omitempty"`                           // 买入到盈亏平衡点比率
	SellToBep             *float64 `protobuf:"fixed64,54,opt,name=sellToBep" json:"sellToBep,omitempty"`                         // 卖出到盈亏平衡点比率
	BuyProfitProbability  *float64 `protobuf:"fixed64,55,opt,name=buyProfitProbability" json:"buyProfitProbability,omitempty"`   // 买入盈利概率
	SellProfitProbability *float64 `protobuf:"fixed64,56,opt,name=sellProfitProbability" json:"sellProfitProbability,omitempty"` // 卖出盈利概率
	IntrinsicValuePer     *float64 `protobuf:"fixed64,57,opt,name=intrinsicValuePer" json:"intrinsicValuePer,omitempty"`         // 内在价值百分比
	TimeValuePer          *float64 `protobuf:"fixed64,58,opt,name=timeValuePer" json:"timeValuePer,omitempty"`                   // 时间价值百分比
	ItmDegree             *float64 `protobuf:"fixed64,59,opt,name=itmDegree" json:"itmDegree,omitempty"`                         // 价内程度
	OtmDegree             *float64 `protobuf:"fixed64,60,opt,name=otmDegree" json:"otmDegree,omitempty"`                         // 价外程度
	OtmProbability        *float64 `protobuf:"fixed64,61,opt,name=otmProbability" json:"otmProbability,omitempty"`               // 价外概率
	SellAnnualizedReturn  *float64 `protobuf:"fixed64,62,opt,name=sellAnnualizedReturn" json:"sellAnnualizedReturn,omitempty"`   // 卖出年化收益率
	IntervalReturn        *float64 `protobuf:"fixed64,63,opt,name=intervalReturn" json:"intervalReturn,omitempty"`               // 卖出区间收益率
	unknownFields         protoimpl.UnknownFields
	sizeCache             protoimpl.SizeCache
}

func (x *OptionScreenItem) Reset() {
	*x = OptionScreenItem{}
	mi := &file_Qot_OptionScreen_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *OptionScreenItem) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OptionScreenItem) ProtoMessage() {}

func (x *OptionScreenItem) ProtoReflect() protoreflect.Message {
	mi := &file_Qot_OptionScreen_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OptionScreenItem.ProtoReflect.Descriptor instead.
func (*OptionScreenItem) Descriptor() ([]byte, []int) {
	return file_Qot_OptionScreen_proto_rawDescGZIP(), []int{9}
}

func (x *OptionScreenItem) GetSecurity() *Security {
	if x != nil {
		return x.Security
	}
	return nil
}

func (x *OptionScreenItem) GetOptionName() string {
	if x != nil && x.OptionName != nil {
		return *x.OptionName
	}
	return ""
}

func (x *OptionScreenItem) GetStrikePrice() float64 {
	if x != nil && x.StrikePrice != nil {
		return *x.StrikePrice
	}
	return 0
}

func (x *OptionScreenItem) GetStrikeDate() int64 {
	if x != nil && x.StrikeDate != nil {
		return *x.StrikeDate
	}
	return 0
}

func (x *OptionScreenItem) GetOptionType() int32 {
	if x != nil && x.OptionType != nil {
		return *x.OptionType
	}
	return 0
}

func (x *OptionScreenItem) GetExerciseType() int32 {
	if x != nil && x.ExerciseType != nil {
		return *x.ExerciseType
	}
	return 0
}

func (x *OptionScreenItem) GetExpirationType() int32 {
	if x != nil && x.ExpirationType != nil {
		return *x.ExpirationType
	}
	return 0
}

func (x *OptionScreenItem) GetInTheMoney() int32 {
	if x != nil && x.InTheMoney != nil {
		return *x.InTheMoney
	}
	return 0
}

func (x *OptionScreenItem) GetLeftDay() int32 {
	if x != nil && x.LeftDay != nil {
		return *x.LeftDay
	}
	return 0
}

func (x *OptionScreenItem) GetPrice() float64 {
	if x != nil && x.Price != nil {
		return *x.Price
	}
	return 0
}

func (x *OptionScreenItem) GetMidPrice() float64 {
	if x != nil && x.MidPrice != nil {
		return *x.MidPrice
	}
	return 0
}

func (x *OptionScreenItem) GetBidPrice() float64 {
	if x != nil && x.BidPrice != nil {
		return *x.BidPrice
	}
	return 0
}

func (x *OptionScreenItem) GetAskPrice() float64 {
	if x != nil && x.AskPrice != nil {
		return *x.AskPrice
	}
	return 0
}

func (x *OptionScreenItem) GetBidAskSpread() float64 {
	if x != nil && x.BidAskSpread != nil {
		return *x.BidAskSpread
	}
	return 0
}

func (x *OptionScreenItem) GetBidVolume() int64 {
	if x != nil && x.BidVolume != nil {
		return *x.BidVolume
	}
	return 0
}

func (x *OptionScreenItem) GetAskVolume() int64 {
	if x != nil && x.AskVolume != nil {
		return *x.AskVolume
	}
	return 0
}

func (x *OptionScreenItem) GetChangeRatio() float64 {
	if x != nil && x.ChangeRatio != nil {
		return *x.ChangeRatio
	}
	return 0
}

func (x *OptionScreenItem) GetVolume() int64 {
	if x != nil && x.Volume != nil {
		return *x.Volume
	}
	return 0
}

func (x *OptionScreenItem) GetTurnover() float64 {
	if x != nil && x.Turnover != nil {
		return *x.Turnover
	}
	return 0
}

func (x *OptionScreenItem) GetOpenInterest() int64 {
	if x != nil && x.OpenInterest != nil {
		return *x.OpenInterest
	}
	return 0
}

func (x *OptionScreenItem) GetBidAskVolumeRatio() float64 {
	if x != nil && x.BidAskVolumeRatio != nil {
		return *x.BidAskVolumeRatio
	}
	return 0
}

func (x *OptionScreenItem) GetOpenInterestMarketCap() float64 {
	if x != nil && x.OpenInterestMarketCap != nil {
		return *x.OpenInterestMarketCap
	}
	return 0
}

func (x *OptionScreenItem) GetVolOIRatio() float64 {
	if x != nil && x.VolOIRatio != nil {
		return *x.VolOIRatio
	}
	return 0
}

func (x *OptionScreenItem) GetPremium() float64 {
	if x != nil && x.Premium != nil {
		return *x.Premium
	}
	return 0
}

func (x *OptionScreenItem) GetImpliedVolatility() float64 {
	if x != nil && x.ImpliedVolatility != nil {
		return *x.ImpliedVolatility
	}
	return 0
}

func (x *OptionScreenItem) GetDelta() float64 {
	if x != nil && x.Delta != nil {
		return *x.Delta
	}
	return 0
}

func (x *OptionScreenItem) GetGamma() float64 {
	if x != nil && x.Gamma != nil {
		return *x.Gamma
	}
	return 0
}

func (x *OptionScreenItem) GetVega() float64 {
	if x != nil && x.Vega != nil {
		return *x.Vega
	}
	return 0
}

func (x *OptionScreenItem) GetTheta() float64 {
	if x != nil && x.Theta != nil {
		return *x.Theta
	}
	return 0
}

func (x *OptionScreenItem) GetRho() float64 {
	if x != nil && x.Rho != nil {
		return *x.Rho
	}
	return 0
}

func (x *OptionScreenItem) GetLeverageRatio() float64 {
	if x != nil && x.LeverageRatio != nil {
		return *x.LeverageRatio
	}
	return 0
}

func (x *OptionScreenItem) GetEffectiveGearing() float64 {
	if x != nil && x.EffectiveGearing != nil {
		return *x.EffectiveGearing
	}
	return 0
}

func (x *OptionScreenItem) GetItmProbability() float64 {
	if x != nil && x.ItmProbability != nil {
		return *x.ItmProbability
	}
	return 0
}

// Deprecated: Marked as deprecated in Qot_OptionScreen.proto.
func (x *OptionScreenItem) GetBuyBreakEvenPoint() float64 {
	if x != nil && x.BuyBreakEvenPoint != nil {
		return *x.BuyBreakEvenPoint
	}
	return 0
}

func (x *OptionScreenItem) GetUnderlyingInfo() *UnderlyingInfo {
	if x != nil {
		return x.UnderlyingInfo
	}
	return nil
}

func (x *OptionScreenItem) GetHistoryVolatility() float64 {
	if x != nil && x.HistoryVolatility != nil {
		return *x.HistoryVolatility
	}
	return 0
}

func (x *OptionScreenItem) GetIvHvRatio() float64 {
	if x != nil && x.IvHvRatio != nil {
		return *x.IvHvRatio
	}
	return 0
}

func (x *OptionScreenItem) GetBuyToBep() float64 {
	if x != nil && x.BuyToBep != nil {
		return *x.BuyToBep
	}
	return 0
}

func (x *OptionScreenItem) GetSellToBep() float64 {
	if x != nil && x.SellToBep != nil {
		return *x.SellToBep
	}
	return 0
}

func (x *OptionScreenItem) GetBuyProfitProbability() float64 {
	if x != nil && x.BuyProfitProbability != nil {
		return *x.BuyProfitProbability
	}
	return 0
}

func (x *OptionScreenItem) GetSellProfitProbability() float64 {
	if x != nil && x.SellProfitProbability != nil {
		return *x.SellProfitProbability
	}
	return 0
}

func (x *OptionScreenItem) GetIntrinsicValuePer() float64 {
	if x != nil && x.IntrinsicValuePer != nil {
		return *x.IntrinsicValuePer
	}
	return 0
}

func (x *OptionScreenItem) GetTimeValuePer() float64 {
	if x != nil && x.TimeValuePer != nil {
		return *x.TimeValuePer
	}
	return 0
}

func (x *OptionScreenItem) GetItmDegree() float64 {
	if x != nil && x.ItmDegree != nil {
		return *x.ItmDegree
	}
	return 0
}

func (x *OptionScreenItem) GetOtmDegree() float64 {
	if x != nil && x.OtmDegree != nil {
		return *x.OtmDegree
	}
	return 0
}

func (x *OptionScreenItem) GetOtmProbability() float64 {
	if x != nil && x.OtmProbability != nil {
		return *x.OtmProbability
	}
	return 0
}

func (x *OptionScreenItem) GetSellAnnualizedReturn() float64 {
	if x != nil && x.SellAnnualizedReturn != nil {
		return *x.SellAnnualizedReturn
	}
	return 0
}

func (x *OptionScreenItem) GetIntervalReturn() float64 {
	if x != nil && x.IntervalReturn != nil {
		return *x.IntervalReturn
	}
	return 0
}

type QotOptionScreenRequest struct {
	state              protoimpl.MessageState `protogen:"open.v1"`
	MarketCategoryList []int32                `protobuf:"varint,1,rep,name=marketCategoryList" json:"marketCategoryList,omitempty"` // MarketCategory 市场品类列表(至少一个)
	FilterList         []*FilterGroup         `protobuf:"bytes,2,rep,name=filterList" json:"filterList,omitempty"`                  // 筛选条件组(组间AND)
	SortList           []*OptionScreenSort    `protobuf:"bytes,3,rep,name=sortList" json:"sortList,omitempty"`                      // 排序
	PageFrom           *int32                 `protobuf:"varint,4,opt,name=pageFrom" json:"pageFrom,omitempty"`                     // 数据偏移位，从0开始
	PageCount          *int32                 `protobuf:"varint,5,opt,name=pageCount" json:"pageCount,omitempty"`                   // 最大数量(最大1000)
	// 返回字段声明(类似StockScreen的retrieve)，值为 OptionIndicatorType/UnderlyingIndicatorType
	// 不填则返回默认基础字段集；code/option_name/strike_date/underlying stock_id 始终返回
	OptionRetrieveList     []OptionIndicatorType     `protobuf:"varint,6,rep,name=optionRetrieveList,enum=futupb.OptionIndicatorType" json:"optionRetrieveList,omitempty"`             // 期权字段, 值为 OptionIndicatorType
	UnderlyingRetrieveList []UnderlyingIndicatorType `protobuf:"varint,7,rep,name=underlyingRetrieveList,enum=futupb.UnderlyingIndicatorType" json:"underlyingRetrieveList,omitempty"` // 标的字段, 值为 UnderlyingIndicatorType
	unknownFields          protoimpl.UnknownFields
	sizeCache              protoimpl.SizeCache
}

func (x *QotOptionScreenRequest) Reset() {
	*x = QotOptionScreenRequest{}
	mi := &file_Qot_OptionScreen_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *QotOptionScreenRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*QotOptionScreenRequest) ProtoMessage() {}

func (x *QotOptionScreenRequest) ProtoReflect() protoreflect.Message {
	mi := &file_Qot_OptionScreen_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use QotOptionScreenRequest.ProtoReflect.Descriptor instead.
func (*QotOptionScreenRequest) Descriptor() ([]byte, []int) {
	return file_Qot_OptionScreen_proto_rawDescGZIP(), []int{10}
}

func (x *QotOptionScreenRequest) GetMarketCategoryList() []int32 {
	if x != nil {
		return x.MarketCategoryList
	}
	return nil
}

func (x *QotOptionScreenRequest) GetFilterList() []*FilterGroup {
	if x != nil {
		return x.FilterList
	}
	return nil
}

func (x *QotOptionScreenRequest) GetSortList() []*OptionScreenSort {
	if x != nil {
		return x.SortList
	}
	return nil
}

func (x *QotOptionScreenRequest) GetPageFrom() int32 {
	if x != nil && x.PageFrom != nil {
		return *x.PageFrom
	}
	return 0
}

func (x *QotOptionScreenRequest) GetPageCount() int32 {
	if x != nil && x.PageCount != nil {
		return *x.PageCount
	}
	return 0
}

func (x *QotOptionScreenRequest) GetOptionRetrieveList() []OptionIndicatorType {
	if x != nil {
		return x.OptionRetrieveList
	}
	return nil
}

func (x *QotOptionScreenRequest) GetUnderlyingRetrieveList() []UnderlyingIndicatorType {
	if x != nil {
		return x.UnderlyingRetrieveList
	}
	return nil
}

type QotOptionScreenResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	LastPage      *bool                  `protobuf:"varint,1,req,name=lastPage" json:"lastPage,omitempty"` // 是否最后一页
	AllCount      *int32                 `protobuf:"varint,2,req,name=allCount" json:"allCount,omitempty"` // 数据总量
	DataList      []*OptionScreenItem    `protobuf:"bytes,3,rep,name=dataList" json:"dataList,omitempty"`  // 返回的期权数据列表
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *QotOptionScreenResponse) Reset() {
	*x = QotOptionScreenResponse{}
	mi := &file_Qot_OptionScreen_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *QotOptionScreenResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*QotOptionScreenResponse) ProtoMessage() {}

func (x *QotOptionScreenResponse) ProtoReflect() protoreflect.Message {
	mi := &file_Qot_OptionScreen_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use QotOptionScreenResponse.ProtoReflect.Descriptor instead.
func (*QotOptionScreenResponse) Descriptor() ([]byte, []int) {
	return file_Qot_OptionScreen_proto_rawDescGZIP(), []int{11}
}

func (x *QotOptionScreenResponse) GetLastPage() bool {
	if x != nil && x.LastPage != nil {
		return *x.LastPage
	}
	return false
}

func (x *QotOptionScreenResponse) GetAllCount() int32 {
	if x != nil && x.AllCount != nil {
		return *x.AllCount
	}
	return 0
}

func (x *QotOptionScreenResponse) GetDataList() []*OptionScreenItem {
	if x != nil {
		return x.DataList
	}
	return nil
}

type QotOptionScreenRequest_Internal struct {
	state         protoimpl.MessageState  `protogen:"open.v1"`
	Payload       *QotOptionScreenRequest `protobuf:"bytes,1,req,name=payload" json:"payload,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *QotOptionScreenRequest_Internal) Reset() {
	*x = QotOptionScreenRequest_Internal{}
	mi := &file_Qot_OptionScreen_proto_msgTypes[12]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *QotOptionScreenRequest_Internal) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*QotOptionScreenRequest_Internal) ProtoMessage() {}

func (x *QotOptionScreenRequest_Internal) ProtoReflect() protoreflect.Message {
	mi := &file_Qot_OptionScreen_proto_msgTypes[12]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use QotOptionScreenRequest_Internal.ProtoReflect.Descriptor instead.
func (*QotOptionScreenRequest_Internal) Descriptor() ([]byte, []int) {
	return file_Qot_OptionScreen_proto_rawDescGZIP(), []int{12}
}

func (x *QotOptionScreenRequest_Internal) GetPayload() *QotOptionScreenRequest {
	if x != nil {
		return x.Payload
	}
	return nil
}

type QotOptionScreenResponse_Internal struct {
	state         protoimpl.MessageState   `protogen:"open.v1"`
	RetType       *RetType                 `protobuf:"varint,1,req,name=retType,enum=futupb.RetType,def=-400" json:"retType,omitempty"` // RetType,返回结果
	RetMsg        *string                  `protobuf:"bytes,2,opt,name=retMsg" json:"retMsg,omitempty"`
	ErrCode       *int32                   `protobuf:"varint,3,opt,name=errCode" json:"errCode,omitempty"`
	Payload       *QotOptionScreenResponse `protobuf:"bytes,4,opt,name=payload" json:"payload,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

// Default values for QotOptionScreenResponse_Internal fields.
const (
	Default_QotOptionScreenResponse_Internal_RetType = RetType_RetType_Unknown
)

func (x *QotOptionScreenResponse_Internal) Reset() {
	*x = QotOptionScreenResponse_Internal{}
	mi := &file_Qot_OptionScreen_proto_msgTypes[13]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *QotOptionScreenResponse_Internal) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*QotOptionScreenResponse_Internal) ProtoMessage() {}

func (x *QotOptionScreenResponse_Internal) ProtoReflect() protoreflect.Message {
	mi := &file_Qot_OptionScreen_proto_msgTypes[13]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use QotOptionScreenResponse_Internal.ProtoReflect.Descriptor instead.
func (*QotOptionScreenResponse_Internal) Descriptor() ([]byte, []int) {
	return file_Qot_OptionScreen_proto_rawDescGZIP(), []int{13}
}

func (x *QotOptionScreenResponse_Internal) GetRetType() RetType {
	if x != nil && x.RetType != nil {
		return *x.RetType
	}
	return Default_QotOptionScreenResponse_Internal_RetType
}

func (x *QotOptionScreenResponse_Internal) GetRetMsg() string {
	if x != nil && x.RetMsg != nil {
		return *x.RetMsg
	}
	return ""
}

func (x *QotOptionScreenResponse_Internal) GetErrCode() int32 {
	if x != nil && x.ErrCode != nil {
		return *x.ErrCode
	}
	return 0
}

func (x *QotOptionScreenResponse_Internal) GetPayload() *QotOptionScreenResponse {
	if x != nil {
		return x.Payload
	}
	return nil
}

var File_Qot_OptionScreen_proto protoreflect.FileDescriptor

const file_Qot_OptionScreen_proto_rawDesc = "" +
	"\n" +
	"\x16Qot_OptionScreen.proto\x12\x06futupb\x1a\vTypes.proto\"H\n" +
	"\x14OptionScreenBoundary\x12\x14\n" +
	"\x05value\x18\x01 \x02(\x01R\x05value\x12\x1a\n" +
	"\bincludes\x18\x02 \x02(\bR\bincludes\"\x8e\x01\n" +
	"\x14OptionScreenInterval\x12:\n" +
	"\tfilterMin\x18\x01 \x01(\v2\x1c.futupb.OptionScreenBoundaryR\tfilterMin\x12:\n" +
	"\tfilterMax\x18\x02 \x01(\v2\x1c.futupb.OptionScreenBoundaryR\tfilterMax\"\x96\x01\n" +
	"\x0eIndicatorValue\x12\x1c\n" +
	"\tvalueList\x18\x01 \x03(\x03R\tvalueList\x12B\n" +
	"\rvalueInterval\x18\x02 \x01(\v2\x1c.futupb.OptionScreenIntervalR\rvalueInterval\x12\"\n" +
	"\fstrValueList\x18\x03 \x03(\tR\fstrValueList\"\xcf\x01\n" +
	"\x13UnderlyingIndicator\x12E\n" +
	"\rindicatorType\x18\x01 \x02(\x0e2\x1f.futupb.UnderlyingIndicatorTypeR\rindicatorType\x12>\n" +
	"\x0eindicatorValue\x18\x02 \x01(\v2\x16.futupb.IndicatorValueR\x0eindicatorValue\x121\n" +
	"\tplateList\x18\x04 \x03(\v2\x13.futupb.OptionPlateR\tplateList\"\x94\x01\n" +
	"\x0fOptionIndicator\x12A\n" +
	"\rindicatorType\x18\x01 \x02(\x0e2\x1b.futupb.OptionIndicatorTypeR\rindicatorType\x12>\n" +
	"\x0eindicatorValue\x18\x02 \x01(\v2\x16.futupb.IndicatorValueR\x0eindicatorValue\"\x8b\x01\n" +
	"\vFilterGroup\x12C\n" +
	"\x0eunderlyingList\x18\x01 \x03(\v2\x1b.futupb.UnderlyingIndicatorR\x0eunderlyingList\x127\n" +
	"\n" +
	"optionList\x18\x02 \x03(\v2\x17.futupb.OptionIndicatorR\n" +
	"optionList\"U\n" +
	"\vOptionPlate\x12$\n" +
	"\rparentPlateId\x18\x01 \x01(\tR\rparentPlateId\x12 \n" +
	"\vplateIdList\x18\x02 \x03(\tR\vplateIdList\"s\n" +
	"\x10OptionScreenSort\x12A\n" +
	"\rindicatorType\x18\x01 \x02(\x0e2\x1b.futupb.OptionIndicatorTypeR\rindicatorType\x12\x1c\n" +
	"\tdirection\x18\x02 \x02(\x05R\tdirection\"\xdc\x01\n" +
	"\x0eUnderlyingInfo\x12\x18\n" +
	"\astockID\x18\x01 \x01(\x04R\astockID\x12\x0e\n" +
	"\x02iv\x18\x02 \x01(\x01R\x02iv\x12\x0e\n" +
	"\x02hv\x18\x03 \x01(\x01R\x02hv\x12\x16\n" +
	"\x06ivRank\x18\x04 \x01(\x01R\x06ivRank\x12\"\n" +
	"\fivPercentile\x18\x05 \x01(\x01R\fivPercentile\x12\x1c\n" +
	"\tmarketCap\x18\x06 \x01(\x01R\tmarketCap\x12\x14\n" +
	"\x05price\x18\a \x01(\x01R\x05price\x12 \n" +
	"\vchangeRatio\x18\b \x01(\x01R\vchangeRatio\"\xae\r\n" +
	"\x10OptionScreenItem\x12,\n" +
	"\bsecurity\x18\x01 \x01(\v2\x10.futupb.SecurityR\bsecurity\x12\x1e\n" +
	"\n" +
	"optionName\x18\x02 \x01(\tR\n" +
	"optionName\x12 \n" +
	"\vstrikePrice\x18\x03 \x01(\x01R\vstrikePrice\x12\x1e\n" +
	"\n" +
	"strikeDate\x18\x04 \x01(\x03R\n" +
	"strikeDate\x12\x1e\n" +
	"\n" +
	"optionType\x18\x05 \x01(\x05R\n" +
	"optionType\x12\"\n" +
	"\fexerciseType\x18\x06 \x01(\x05R\fexerciseType\x12&\n" +
	"\x0eexpirationType\x18\a \x01(\x05R\x0eexpirationType\x12\x1e\n" +
	"\n" +
	"inTheMoney\x18\b \x01(\x05R\n" +
	"inTheMoney\x12\x18\n" +
	"\aleftDay\x18\t \x01(\x05R\aleftDay\x12\x14\n" +
	"\x05price\x18\x14 \x01(\x01R\x05price\x12\x1a\n" +
	"\bmidPrice\x18\x15 \x01(\x01R\bmidPrice\x12\x1a\n" +
	"\bbidPrice\x18\x16 \x01(\x01R\bbidPrice\x12\x1a\n" +
	"\baskPrice\x18\x17 \x01(\x01R\baskPrice\x12\"\n" +
	"\fbidAskSpread\x18\x18 \x01(\x01R\fbidAskSpread\x12\x1c\n" +
	"\tbidVolume\x18\x19 \x01(\x03R\tbidVolume\x12\x1c\n" +
	"\taskVolume\x18\x1a \x01(\x03R\taskVolume\x12 \n" +
	"\vchangeRatio\x18\x1b \x01(\x01R\vchangeRatio\x12\x16\n" +
	"\x06volume\x18\x1c \x01(\x03R\x06volume\x12\x1a\n" +
	"\bturnover\x18\x1d \x01(\x01R\bturnover\x12\"\n" +
	"\fopenInterest\x18\x1e \x01(\x03R\fopenInterest\x12,\n" +
	"\x11bidAskVolumeRatio\x18\x1f \x01(\x01R\x11bidAskVolumeRatio\x124\n" +
	"\x15openInterestMarketCap\x18  \x01(\x01R\x15openInterestMarketCap\x12\x1e\n" +
	"\n" +
	"volOIRatio\x18! \x01(\x01R\n" +
	"volOIRatio\x12\x18\n" +
	"\apremium\x18\" \x01(\x01R\apremium\x12,\n" +
	"\x11impliedVolatility\x18( \x01(\x01R\x11impliedVolatility\x12\x14\n" +
	"\x05delta\x18) \x01(\x01R\x05delta\x12\x14\n" +
	"\x05gamma\x18* \x01(\x01R\x05gamma\x12\x12\n" +
	"\x04vega\x18+ \x01(\x01R\x04vega\x12\x14\n" +
	"\x05theta\x18, \x01(\x01R\x05theta\x12\x10\n" +
	"\x03rho\x18- \x01(\x01R\x03rho\x12$\n" +
	"\rleverageRatio\x18. \x01(\x01R\rleverageRatio\x12*\n" +
	"\x10effectiveGearing\x18/ \x01(\x01R\x10effectiveGearing\x12&\n" +
	"\x0eitmProbability\x180 \x01(\x01R\x0eitmProbability\x120\n" +
	"\x11buyBreakEvenPoint\x181 \x01(\x01B\x02\x18\x01R\x11buyBreakEvenPoint\x12>\n" +
	"\x0eunderlyingInfo\x182 \x01(\v2\x16.futupb.UnderlyingInfoR\x0eunderlyingInfo\x12,\n" +
	"\x11historyVolatility\x183 \x01(\x01R\x11historyVolatility\x12\x1c\n" +
	"\tivHvRatio\x184 \x01(\x01R\tivHvRatio\x12\x1a\n" +
	"\bbuyToBep\x185 \x01(\x01R\bbuyToBep\x12\x1c\n" +
	"\tsellToBep\x186 \x01(\x01R\tsellToBep\x122\n" +
	"\x14buyProfitProbability\x187 \x01(\x01R\x14buyProfitProbability\x124\n" +
	"\x15sellProfitProbability\x188 \x01(\x01R\x15sellProfitProbability\x12,\n" +
	"\x11intrinsicValuePer\x189 \x01(\x01R\x11intrinsicValuePer\x12\"\n" +
	"\ftimeValuePer\x18: \x01(\x01R\ftimeValuePer\x12\x1c\n" +
	"\titmDegree\x18; \x01(\x01R\titmDegree\x12\x1c\n" +
	"\totmDegree\x18< \x01(\x01R\totmDegree\x12&\n" +
	"\x0eotmProbability\x18= \x01(\x01R\x0eotmProbability\x122\n" +
	"\x14sellAnnualizedReturn\x18> \x01(\x01R\x14sellAnnualizedReturn\x12&\n" +
	"\x0eintervalReturn\x18? \x01(\x01R\x0eintervalReturn\"\x93\x03\n" +
	"\x16QotOptionScreenRequest\x12.\n" +
	"\x12marketCategoryList\x18\x01 \x03(\x05R\x12marketCategoryList\x123\n" +
	"\n" +
	"filterList\x18\x02 \x03(\v2\x13.futupb.FilterGroupR\n" +
	"filterList\x124\n" +
	"\bsortList\x18\x03 \x03(\v2\x18.futupb.OptionScreenSortR\bsortList\x12\x1a\n" +
	"\bpageFrom\x18\x04 \x01(\x05R\bpageFrom\x12\x1c\n" +
	"\tpageCount\x18\x05 \x01(\x05R\tpageCount\x12K\n" +
	"\x12optionRetrieveList\x18\x06 \x03(\x0e2\x1b.futupb.OptionIndicatorTypeR\x12optionRetrieveList\x12W\n" +
	"\x16underlyingRetrieveList\x18\a \x03(\x0e2\x1f.futupb.UnderlyingIndicatorTypeR\x16underlyingRetrieveList\"\x87\x01\n" +
	"\x17QotOptionScreenResponse\x12\x1a\n" +
	"\blastPage\x18\x01 \x02(\bR\blastPage\x12\x1a\n" +
	"\ballCount\x18\x02 \x02(\x05R\ballCount\x124\n" +
	"\bdataList\x18\x03 \x03(\v2\x18.futupb.OptionScreenItemR\bdataList\"[\n" +
	"\x1fQotOptionScreenRequest_Internal\x128\n" +
	"\apayload\x18\x01 \x02(\v2\x1e.futupb.QotOptionScreenRequestR\apayload\"\xcb\x01\n" +
	" QotOptionScreenResponse_Internal\x12:\n" +
	"\aretType\x18\x01 \x02(\x0e2\x0f.futupb.RetType:\x0fRetType_UnknownR\aretType\x12\x16\n" +
	"\x06retMsg\x18\x02 \x01(\tR\x06retMsg\x12\x18\n" +
	"\aerrCode\x18\x03 \x01(\x05R\aerrCode\x129\n" +
	"\apayload\x18\x04 \x01(\v2\x1f.futupb.QotOptionScreenResponseR\apayload*\xdc\x01\n" +
	"\x0eMarketCategory\x12\x1b\n" +
	"\x17MarketCategory_US_Stock\x10\x00\x12\x1b\n" +
	"\x17MarketCategory_US_Index\x10\x01\x12\x1c\n" +
	"\x18MarketCategory_US_Future\x10\x02\x12\x1b\n" +
	"\x17MarketCategory_HK_Stock\x10\x03\x12\x1b\n" +
	"\x17MarketCategory_HK_Index\x10\x04\x12\x1b\n" +
	"\x17MarketCategory_JP_Stock\x10\x05\x12\x1b\n" +
	"\x17MarketCategory_JP_Index\x10\x06*\xb4\x05\n" +
	"\x17UnderlyingIndicatorType\x12#\n" +
	"\x1fUnderlyingIndicatorType_Unknown\x10\x00\x12%\n" +
	"!UnderlyingIndicatorType_StockList\x10e\x12%\n" +
	"\x1dUnderlyingIndicatorType_Plate\x10g\x1a\x02\b\x01\x12%\n" +
	"!UnderlyingIndicatorType_IndexList\x10j\x12#\n" +
	"\x1eUnderlyingIndicatorType_Volume\x10\xc9\x01\x12)\n" +
	"$UnderlyingIndicatorType_OpenInterest\x10\xca\x01\x12\x1f\n" +
	"\x1aUnderlyingIndicatorType_IV\x10\xcb\x01\x12\x1f\n" +
	"\x1aUnderlyingIndicatorType_HV\x10\xcc\x01\x12#\n" +
	"\x1eUnderlyingIndicatorType_IVRank\x10\xcd\x01\x12)\n" +
	"$UnderlyingIndicatorType_IVPercentile\x10\xce\x01\x12%\n" +
	" UnderlyingIndicatorType_IVChange\x10\xcf\x01\x12*\n" +
	"%UnderlyingIndicatorType_IVChangeRatio\x10\xd0\x01\x12&\n" +
	"!UnderlyingIndicatorType_IVHVRatio\x10\xd1\x01\x12'\n" +
	"\"UnderlyingIndicatorType_IVHVSpread\x10\xd2\x01\x12&\n" +
	"!UnderlyingIndicatorType_MarketCap\x10\x91\x03\x12'\n" +
	"\"UnderlyingIndicatorType_StockPrice\x10\x92\x03\x12(\n" +
	"#UnderlyingIndicatorType_ChangeRatio\x10\x93\x03*\xf9\r\n" +
	"\x13OptionIndicatorType\x12\x1f\n" +
	"\x1bOptionIndicatorType_Unknown\x10\x00\x12$\n" +
	"\x1fOptionIndicatorType_StrikePrice\x10\xe9\a\x12 \n" +
	"\x1bOptionIndicatorType_LeftDay\x10\xea\a\x12#\n" +
	"\x1eOptionIndicatorType_OptionType\x10\xeb\a\x12%\n" +
	" OptionIndicatorType_ExerciseType\x10\xec\a\x12'\n" +
	"\"OptionIndicatorType_ExpirationType\x10\xed\a\x12,\n" +
	"'OptionIndicatorType_StrikeDateTimestamp\x10\xef\a\x12#\n" +
	"\x1eOptionIndicatorType_InTheMoney\x10\xd1\x0f\x12\x1e\n" +
	"\x19OptionIndicatorType_Price\x10\xd2\x0f\x12!\n" +
	"\x1cOptionIndicatorType_MidPrice\x10\xd3\x0f\x12!\n" +
	"\x1cOptionIndicatorType_BidPrice\x10\xd4\x0f\x12!\n" +
	"\x1cOptionIndicatorType_AskPrice\x10\xd5\x0f\x12%\n" +
	" OptionIndicatorType_BidAskSpread\x10\xd6\x0f\x12\"\n" +
	"\x1dOptionIndicatorType_BidVolume\x10\xd7\x0f\x12\"\n" +
	"\x1dOptionIndicatorType_AskVolume\x10\xd8\x0f\x12*\n" +
	"%OptionIndicatorType_BidAskVolumeRatio\x10\xd9\x0f\x12$\n" +
	"\x1fOptionIndicatorType_ChangeRatio\x10\xda\x0f\x12\x1f\n" +
	"\x1aOptionIndicatorType_Volume\x10\xdb\x0f\x12!\n" +
	"\x1cOptionIndicatorType_Turnover\x10\xdc\x0f\x12%\n" +
	" OptionIndicatorType_OpenInterest\x10\xdd\x0f\x12.\n" +
	")OptionIndicatorType_OpenInterestMarketCap\x10\xde\x0f\x12#\n" +
	"\x1eOptionIndicatorType_VolOIRatio\x10\xe2\x0f\x12 \n" +
	"\x1bOptionIndicatorType_Premium\x10\xe5\x0f\x12*\n" +
	"%OptionIndicatorType_ImpliedVolatility\x10\xb9\x17\x12*\n" +
	"%OptionIndicatorType_HistoryVolatility\x10\xba\x17\x12\"\n" +
	"\x1dOptionIndicatorType_IVHVRatio\x10\xbb\x17\x12\x1e\n" +
	"\x19OptionIndicatorType_Delta\x10\xbc\x17\x12\x1e\n" +
	"\x19OptionIndicatorType_Gamma\x10\xbd\x17\x12\x1d\n" +
	"\x18OptionIndicatorType_Vega\x10\xbe\x17\x12\x1e\n" +
	"\x19OptionIndicatorType_Theta\x10\xbf\x17\x12\x1c\n" +
	"\x17OptionIndicatorType_Rho\x10\xc0\x17\x12&\n" +
	"!OptionIndicatorType_LeverageRatio\x10\xc1\x17\x12)\n" +
	"$OptionIndicatorType_EffectiveGearing\x10\xc2\x17\x12!\n" +
	"\x1cOptionIndicatorType_BuyToBep\x10\xc3\x17\x12\"\n" +
	"\x1dOptionIndicatorType_SellToBep\x10\xc4\x17\x12-\n" +
	"(OptionIndicatorType_BuyProfitProbability\x10\xc5\x17\x12.\n" +
	")OptionIndicatorType_SellProfitProbability\x10\xc6\x17\x12*\n" +
	"%OptionIndicatorType_IntrinsicValuePer\x10\xc7\x17\x12%\n" +
	" OptionIndicatorType_TimeValuePer\x10\xc8\x17\x12\"\n" +
	"\x1dOptionIndicatorType_ITMDegree\x10\xc9\x17\x12\"\n" +
	"\x1dOptionIndicatorType_OTMDegree\x10\xca\x17\x12'\n" +
	"\"OptionIndicatorType_ITMProbability\x10\xcb\x17\x12'\n" +
	"\"OptionIndicatorType_OTMProbability\x10\xcc\x17\x12-\n" +
	"(OptionIndicatorType_SellAnnualizedReturn\x10\xcd\x17\x12'\n" +
	"\"OptionIndicatorType_IntervalReturn\x10\xce\x17\x12.\n" +
	"%OptionIndicatorType_BuyBreakEvenPoint\x10\xcf\x17\x1a\x02\b\x01B\x1fZ\x1dgithub.com/santsai/futu-go/pb"

var (
	file_Qot_OptionScreen_proto_rawDescOnce sync.Once
	file_Qot_OptionScreen_proto_rawDescData []byte
)

func file_Qot_OptionScreen_proto_rawDescGZIP() []byte {
	file_Qot_OptionScreen_proto_rawDescOnce.Do(func() {
		file_Qot_OptionScreen_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_Qot_OptionScreen_proto_rawDesc), len(file_Qot_OptionScreen_proto_rawDesc)))
	})
	return file_Qot_OptionScreen_proto_rawDescData
}

var file_Qot_OptionScreen_proto_enumTypes = make([]protoimpl.EnumInfo, 3)
var file_Qot_OptionScreen_proto_msgTypes = make([]protoimpl.MessageInfo, 14)
var file_Qot_OptionScreen_proto_goTypes = []any{
	(MarketCategory)(0),                      // 0: futupb.MarketCategory
	(UnderlyingIndicatorType)(0),             // 1: futupb.UnderlyingIndicatorType
	(OptionIndicatorType)(0),                 // 2: futupb.OptionIndicatorType
	(*OptionScreenBoundary)(nil),             // 3: futupb.OptionScreenBoundary
	(*OptionScreenInterval)(nil),             // 4: futupb.OptionScreenInterval
	(*IndicatorValue)(nil),                   // 5: futupb.IndicatorValue
	(*UnderlyingIndicator)(nil),              // 6: futupb.UnderlyingIndicator
	(*OptionIndicator)(nil),                  // 7: futupb.OptionIndicator
	(*FilterGroup)(nil),                      // 8: futupb.FilterGroup
	(*OptionPlate)(nil),                      // 9: futupb.OptionPlate
	(*OptionScreenSort)(nil),                 // 10: futupb.OptionScreenSort
	(*UnderlyingInfo)(nil),                   // 11: futupb.UnderlyingInfo
	(*OptionScreenItem)(nil),                 // 12: futupb.OptionScreenItem
	(*QotOptionScreenRequest)(nil),           // 13: futupb.QotOptionScreenRequest
	(*QotOptionScreenResponse)(nil),          // 14: futupb.QotOptionScreenResponse
	(*QotOptionScreenRequest_Internal)(nil),  // 15: futupb.QotOptionScreenRequest_Internal
	(*QotOptionScreenResponse_Internal)(nil), // 16: futupb.QotOptionScreenResponse_Internal
	(*Security)(nil),                         // 17: futupb.Security
	(RetType)(0),                             // 18: futupb.RetType
}
var file_Qot_OptionScreen_proto_depIdxs = []int32{
	3,  // 0: futupb.OptionScreenInterval.filterMin:type_name -> futupb.OptionScreenBoundary
	3,  // 1: futupb.OptionScreenInterval.filterMax:type_name -> futupb.OptionScreenBoundary
	4,  // 2: futupb.IndicatorValue.valueInterval:type_name -> futupb.OptionScreenInterval
	1,  // 3: futupb.UnderlyingIndicator.indicatorType:type_name -> futupb.UnderlyingIndicatorType
	5,  // 4: futupb.UnderlyingIndicator.indicatorValue:type_name -> futupb.IndicatorValue
	9,  // 5: futupb.UnderlyingIndicator.plateList:type_name -> futupb.OptionPlate
	2,  // 6: futupb.OptionIndicator.indicatorType:type_name -> futupb.OptionIndicatorType
	5,  // 7: futupb.OptionIndicator.indicatorValue:type_name -> futupb.IndicatorValue
	6,  // 8: futupb.FilterGroup.underlyingList:type_name -> futupb.UnderlyingIndicator
	7,  // 9: futupb.FilterGroup.optionList:type_name -> futupb.OptionIndicator
	2,  // 10: futupb.OptionScreenSort.indicatorType:type_name -> futupb.OptionIndicatorType
	17, // 11: futupb.OptionScreenItem.security:type_name -> futupb.Security
	11, // 12: futupb.OptionScreenItem.underlyingInfo:type_name -> futupb.UnderlyingInfo
	8,  // 13: futupb.QotOptionScreenRequest.filterList:type_name -> futupb.FilterGroup
	10, // 14: futupb.QotOptionScreenRequest.sortList:type_name -> futupb.OptionScreenSort
	2,  // 15: futupb.QotOptionScreenRequest.optionRetrieveList:type_name -> futupb.OptionIndicatorType
	1,  // 16: futupb.QotOptionScreenRequest.underlyingRetrieveList:type_name -> futupb.UnderlyingIndicatorType
	12, // 17: futupb.QotOptionScreenResponse.dataList:type_name -> futupb.OptionScreenItem
	13, // 18: futupb.QotOptionScreenRequest_Internal.payload:type_name -> futupb.QotOptionScreenRequest
	18, // 19: futupb.QotOptionScreenResponse_Internal.retType:type_name -> futupb.RetType
	14, // 20: futupb.QotOptionScreenResponse_Internal.payload:type_name -> futupb.QotOptionScreenResponse
	21, // [21:21] is the sub-list for method output_type
	21, // [21:21] is the sub-list for method input_type
	21, // [21:21] is the sub-list for extension type_name
	21, // [21:21] is the sub-list for extension extendee
	0,  // [0:21] is the sub-list for field type_name
}

func init() { file_Qot_OptionScreen_proto_init() }
func file_Qot_OptionScreen_proto_init() {
	if File_Qot_OptionScreen_proto != nil {
		return
	}
	file_Types_proto_init()
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_Qot_OptionScreen_proto_rawDesc), len(file_Qot_OptionScreen_proto_rawDesc)),
			NumEnums:      3,
			NumMessages:   14,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_Qot_OptionScreen_proto_goTypes,
		DependencyIndexes: file_Qot_OptionScreen_proto_depIdxs,
		EnumInfos:         file_Qot_OptionScreen_proto_enumTypes,
		MessageInfos:      file_Qot_OptionScreen_proto_msgTypes,
	}.Build()
	File_Qot_OptionScreen_proto = out.File
	file_Qot_OptionScreen_proto_goTypes = nil
	file_Qot_OptionScreen_proto_depIdxs = nil
}
